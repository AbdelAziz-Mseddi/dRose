...
def delete():
    ...
    console.print(
        "[#FA5C5C]This will permanently delete all download history.[/#FA5C5C]"
    )
...
def playlists(
    ...
    try:
        ...
        items = (
            repo.get_downloaded_playlists() if downloaded else repo.get_all_playlists()
        )

        if not items:
            message = (
                "No downloaded playlists found yet."
                if downloaded
                else "No playlists found yet."
            )

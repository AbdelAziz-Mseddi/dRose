...
def set(
    output_folder: str = typer.Option(
        None, "--output-folder", "-o", help="Default output folder for downloads"
    ),
    audio_format: str = typer.Option(
        None, "--audio-format", "-f", help="Default audio format"
    ),
):
    ...
    if all(v is None for v in updates.values()):
        console.print(
            "[#FA5C5C]No settings provided. Use --output-folder or --audio-format.[/#FA5C5C]"
        )
        raise typer.Exit(1)
    ...
    if output_folder is not None:
        console.print(
            f"[#A8DF8E]ø Output Folder set to: {merged[0].get('output_folder')}[/#A8DF8E]"
        )
    if audio_format is not None:
        console.print(
            f"[#A8DF8E]ø Audio Format set to: {merged[0].get('audio_format')}[/#A8DF8E]"
        )
...
def get(
    setting: str = typer.Argument(
        ..., help="Setting name (output_folder or audio_format)"
    ),
):
...
def show():
    ...
    if eq:
        console.print(
            "[#A8DF8E]Default configuration is set as the Current Configuration[/#A8DF8E]"
        )
...
def reset():
    ...
    if eq:
        console.print(
            "[#A8DF8E]Configuration is already :3 at default values 🌹[/#A8DF8E]"
        )
    else:
        console.print("[#FFA240]Resetting to default configuration...[/#FFA240]")
        conf.reset_config()
        console.print(
            "[#A8DF8E]✓ Configuration reset to defaults successfully![/#A8DF8E]"
        )
